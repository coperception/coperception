    ...
    def forward(self, bevs, trans_matrices, num_agent_tensor, batch_size=1):
        ...
        for b in range(batch_size):
            ...
            for _ in range(self.gnn_iter_num):
                ...
                # compress & decompress
                feat_maps = F.relu(self.u_encoder.bn_compress(self.u_encoder.com_compresser(feat_maps)))
                feat_maps = F.relu(self.u_encoder.bn_decompress(self.u_encoder.com_decompresser(feat_maps)))
        ...
        decoded_layers = super().get_decoded_layers(encoded_layers, feat_maps, batch_size)
        x = decoded_layers[0]
