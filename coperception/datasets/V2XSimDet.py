...
class V2XSimDet(Dataset):
    def __init__(self, dataset_roots=None, config=None, config_global=None, split=None, cache_size=10000, val=False, bound=None, kd_flag=False, no_cross_road=False):
        """
    ...
    def pick_single_agent(self, agent_id, idx):
        ...
        if idx in self.cache[agent_id]:
            ...
            if gt_data_handle == 0:
                ...
                if self.kd_flag or self.bound == 'upperbound':
                    padded_voxel_points_teacher = np.zeros_like(self.padded_voxel_points_meta)

                if self.val:
                    return padded_voxel_points, padded_voxel_points_teacher, label_one_hot, \
                           reg_target, reg_loss_mask, anchors_map, vis_maps, [
                               {"gt_box": []}], [seq_file], \
                           0, 0, np.zeros((self.num_agent, 4, 4))
                else:
                    return padded_voxel_points, padded_voxel_points_teacher, label_one_hot, reg_target, reg_loss_mask, anchors_map, vis_maps, 0, 0, np.zeros(
                        (self.num_agent, 4, 4))
            else:
        ...
        if empty_flag == False:
            ...
            padded_voxel_points_teacher = []
            if 'voxel_indices_teacher' in gt_dict and (self.kd_flag or self.bound == 'upperbound' or self.bound == 'both'):
                if self.no_cross_road:
